<template>
  <main class="main">
    ...
    <section class="content" v-if="!err">
        ...
        <table class="table" v-if="this.load">
          ...
          <tbody>
            ...
            <tr>
              <td v-if="this.pod == 'd'">
                <img
                  title="День"
                  class="pod__img"
                  src="http://icons.iconarchive.com/icons/jaan-jaak/weather/256/cloudy-partly-icon.png"
                  alt="day"
                >
              </td>
              <td v-else>
                <img
                  title="Ночь"
                  class="pod__img"
                  src="http://www.pf-arkhbum.ru/skin/img/icons/weather/night/02n.png"
                  alt="night"
                >
              </td>
              <td>
                <h2>{{this.data.country_code}} {{this.data.city_name}}</h2>
              </td>
            </tr>
            <!-- Описание погоды -->
            <tr>
            ...
            <tr>
              <th>Ветер:</th>
              <td>
                {{this.data.wind_cdir_full}}
                <br>
                {{this.data.wind_spd}} м/с
              </td>
            </tr>
            <!-- Восход заход солнца -->
            <!-- <tr>
      ...
      <div class="lds-ripple" v-if="!this.load">
        <div></div>
        <div></div>
      </div>
    </section>
    ...
    <section class="err" v-else>
      {{errorMsg.cod}} {{errorMsg.message}}
      <br>
      {{errorMsg}}
      <div class="geo-err" v-if="geoErr">
        <img
          src="http://smartphonus.com/wp-content/uploads/2017/08/081417_0836_3.png"
          alt="geoErr"
          class="geo-err__img"
        >
        <img
          src="http://smartphonus.com/wp-content/uploads/2017/08/081417_0836_4.png"
          alt="geoErr"
          class="geo-err__img"
        >
      </div>
  ...
  methods: {
    ...
    async reqAxios(pos) {
      ...
      localStorage.setItem(
        "geo",
        JSON.stringify({ lat: this.latitude, lon: this.longitude })
      );
      //
      await axios
        .get(
          `https://api.weatherbit.io/v2.0/current?&lat=${this.latitude}&lon=${
            this.longitude
          }&key=${this.apiKey}&lang=ru`
        )
        .then(res => {
    ...
    errGeoloc(err) {
      if (err.code == 1) {
        ...
        this.geoErr = true;
        this.errorMsg =
          "Хороший программист,Неее Очень...Возможно вам поможет эта инструкция";
      } else {
        this.err = false;
        this.errorMsg = "";
      }
    ...
    bgChange(code) {
      switch (code) {
        ...
        case "623":
          // Снегь
          this.imgSrc =
            "http://media.nyckelpiganjenny.se/2015/12/star-winter.jpg";
          break;
        case "700":
        case "711":
        case "721":
        ...
        case "804":
          // Солнце
          this.imgSrc =
            "https://avatars.mds.yandex.net/get-pdb/27625/7eee2880-7d86-481b-91b5-d49d59ce807e/s800";
          break;
        default:
          this.imgSrc =
            "http://www.gvirt.com/uploads/posts/2014-04/1396979891_photo1572242-from-lostpic.net-with-love.jpg";
          break;
      }
    }
  },
